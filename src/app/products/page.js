...
const fetchProducts = async (categoryId) => {
    try {
        ...
        return categoryId === 0 ? response.data.products || [] : response.data.product || [];
    } catch (error) {
            ...
            <div className="product-head flex flex-col lg:flex-row items-center mt-12 h-[150px] lg:h-[200px]">
                    <div className="flex w-full justify-between items-center relative h-full">
                        ...
                        <div className="flex flex-col items-center mx-auto h-full justify-center">
                            <Breadcrumb
                                separator={<span className="mx-4" style={{ color: "#1e874c", fontSize: "24px" }}>&gt;</span>}
                            >
                ...
                <div className="products-container p-6 mx-4 sm:mx-6 lg:mx-14 lg:px-14 mt-1">
                            ...
                            label: (
                                <div className="flex flex-col items-center min-w-[80px] sm:min-w-[100px]">
                                    <Image
                                        ...
                                        width={70}
                                        height={70}
                                        className={`product-tab ${activeTab === category.id ? "active-tab-products" : ""}`}
                                        loading="lazy"
                                    />
                                    <span className="mt-1 category-name text-center">{category.name}</span>
                                </div>
                            ),
                            key: category.id,
                        }))}
                    ...
                    {loading ? (
                        ...
                        <p className="text-center text-gray-500 mt-4">No products available for this category.</p>
                    )}
                </div>
            </div>
        </>
